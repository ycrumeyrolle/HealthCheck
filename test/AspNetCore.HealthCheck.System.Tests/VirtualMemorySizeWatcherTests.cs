...
{
    public class VirtualMemorySizeWatcherTests
    {
        [Theory]
        ...
        public async Task CheckHealthAsync_Healthy_CheckSucceeded(long virtualMemorySize, long errorThreshold, long warningThreshold)
        {
            ...
            var watcher = new VirtualMemorySizeWatcher(virtualMemorySizeProvider);

            var settings = new FloorThresholdWatchSettings("disk space", false, 0, null, errorThreshold, warningThreshold);
        ...
        public async Task CheckHealthAsync_Unhealthy_CheckFailed(long virtualMemorySize, long errorThreshold, long warningThreshold)
        {
            ...
            var settings = new FloorThresholdWatchSettings("virtual memory size", false, 0, null, errorThreshold, warningThreshold);
            var context = new HealthContext(settings);
        ...
        public async Task CheckHealthAsync_Unhealthy_ThrowsException()
        {
            ...
            var watcher = new VirtualMemorySizeWatcher(virtualMemorySizeProvider.Object);

            var settings = new FloorThresholdWatchSettings("virtual memory size", false, 0, null, 0, 0);
            var context = new HealthContext(settings);
